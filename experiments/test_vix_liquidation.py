...
def custom_run(self):
    ...
    for i, date in enumerate(all_dates):
        ...
        # If VIX > 30 on previous day, liquidate everything!
        vix_prev = 0
        if prev_date in vix_df.index:
            vix_prev = vix_df.loc[prev_date, 'Close']
            
        if vix_prev > 30.0 and len(self.open_trades) > 0:
        ...
        regime_allowed = True
        if prev_date in vix_df.index:
            vix_mult = vix_df.loc[prev_date].get('VIX_Multiplier', 1.0)
        ...
        for stock, today_row in candidates:
            ...
            shares, sl = strategy.calculate_position_size(
                ...
                entry_price=today_row['Open'], atr=prev_row.get('ATR', 0), vix_multiplier=vix_mult
            )
            if shares > 0:
...
print("\n--- RESULTS WITH VIX > 30 LIQUIDATION ---")
