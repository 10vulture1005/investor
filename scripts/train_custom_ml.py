...
def build_dataset():
    logger.info("Fetching data for Custom ML dataset...")
    # Using 2017 to 2025 for maximum training data
    ...
    for ticker in CUSTOM_STOCKS.values():
        df = raw_data_dict.get(ticker, pd.DataFrame())
        if df.empty:
            continue
...
def train_model(df_ml):
    logger.info("Training Custom XGBoost Classifier...")
    ...
    joblib.dump(model, 'models/custom_xgb_filter.pkl')
    logger.info("Model saved to models/custom_xgb_filter.pkl")
